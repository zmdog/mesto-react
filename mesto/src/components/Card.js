    ...
    if (card) {
        return (
                <article className="element">
                    {isOwn && <button aria-label={`Удалить ${card.name}`} onClick={handleDeleteCard} type="button" className="button element__delete"/>}
                    ...
                    <div className="element__label">
                        ...
                        <div className="element__group">
                            <button aria-label="" onClick={()=>onLikeClick(card, isLiked)} type="button" className={`button element__like ${isLiked && 'element__like_active'}`}/>
